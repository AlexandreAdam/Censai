...
class AUTOENCODER(tf.keras.Model):
    def __init__(self, loadsave_file_name = "checkpoints/AE_weights"):
        super(AUTOENCODER, self).__init__()

        activation = lrelu4p
        D01 = tf.keras.layers.Conv2D(32 , (3, 3), activation=activation, strides=1 , padding='same')
        D02 = tf.keras.layers.Conv2D(64 , (6, 6), activation=activation, strides=4 , padding='same') 
        D03 = tf.keras.layers.Conv2D(64 , (3, 3), activation=activation, strides=1 , padding='same')
        D04 = tf.keras.layers.Conv2D(64 , (6, 6), activation=activation, strides=4 , padding='same')
        D05 = tf.keras.layers.Conv2D(64 , (3, 3), activation=activation, strides=1 , padding='same')
        D06 = tf.keras.layers.Conv2D(64, (8, 8), activation=activation, strides=2 , padding='same') 
        D07 = tf.keras.layers.Conv2D(128, (16, 16), activation=activation, strides=1 , padding='valid') 
        self.encode_layers = [D01, D02, D03, D04, D05, D06, D07]

        U01 = tf.keras.layers.Conv2DTranspose(128, (8, 8), activation=activation, strides=4, padding='same') 
        U02 = tf.keras.layers.Conv2DTranspose(128, (8, 8), activation=activation, strides=4, padding='same')
        U03 = tf.keras.layers.Conv2DTranspose(64, (6, 6), activation=activation, strides=2, padding='same') 
        U04 = tf.keras.layers.Conv2DTranspose(64, (6, 6), activation=activation, strides=1, padding='same')
        U05 = tf.keras.layers.Conv2DTranspose(64, (6, 6), activation=activation, strides=4, padding='same') 
        U06 = tf.keras.layers.Conv2DTranspose(64, (6, 6), activation=activation, strides=1, padding='same')
        U07 = tf.keras.layers.Conv2DTranspose(64, (6, 6), activation=activation, strides=2, padding='same') 
        U08 = tf.keras.layers.Conv2DTranspose(64, (6, 6), activation=activation, strides=1, padding='same')
        U09 = tf.keras.layers.Conv2DTranspose(32, (6, 6), activation=activation, strides=2, padding='same') 
        U010 = tf.keras.layers.Conv2DTranspose(16, (4, 4), activation='linear', strides=1, padding='same')
        U011 = tf.keras.layers.Conv2DTranspose(1, (3, 3), activation='linear', strides=1, padding='same')
        self.decode_layers = [U01, U02, U03, U04, U05, U06, U07, U08, U09, U010, U011]
        self.AE_checkpoint_path = loadsave_file_name
...
class FFCNN(tf.keras.Model):
    def __init__(self , load_checkpoint_path = "checkpoints/FFCNN_weights" , save_checkpoint_path = "checkpoints/FFCNN_weights", AUTOENCODER_obj = AUTOENCODER() ):
        ...
        D04 = tf.keras.layers.Conv2D(64 , (6, 6), activation=activation, strides=4 , padding='same')
        D05 = tf.keras.layers.Conv2D(64 , (3, 3), activation=activation, strides=1 , padding='same')
        D06 = tf.keras.layers.Conv2D(64, (6, 6), activation=activation, strides=4 , padding='same') 
        D07 = tf.keras.layers.Conv2D(64, (8, 8), activation=activation, strides=1 , padding='valid') 
        self.FP_layers = [D01, D02, D03, D04, D05, D06, D07]
        self.AE = AUTOENCODER_obj
        #self.AE.Load()
    ...
    def cost_function(self , X , Y):
        prediction = self.fullforward_pass(X)
        prediction = tf.reshape(prediction, (-1,512,512,1))
        cost = tf.reduce_mean((prediction - Y)**2)
        return cost
        
    def train(self , X , Y , optimizer):
        with tf.GradientTape() as tape:
            tape.watch(self.variables)
            cost_value = self.cost_function(X,Y)
        weight_grads = tape.gradient(cost_value, self.variables  )
    ...
    def Save(self):
        self.save_weights(self.save_checkpoint_path)

    def Load(self):
        self.load_weights(self.load_checkpoint_path)
        
        
        
        
def lrelu2(x, alpha=0.01):
    return tf.maximum(x, tf.multiply(x, alpha))
...
class RIM_COMP_DECOMP(tf.keras.Model):
    def __init__(self):
        super(RIM_COMP_DECOMP, self).__init__()
        ...
        self.comp_layers = [D01, D02]
        
        U01 = tf.keras.layers.Conv2DTranspose(32, (6, 6), activation=activation, strides=self.STRIDE, padding='same') 
        U02 = tf.keras.layers.Conv2DTranspose(32, (6, 6), activation=activation, strides=self.STRIDE, padding='same') 
        self.decomp_layers = [U01, U02]
    ...
    def DECOMP(self, X):
        for layer in self.decomp_layers:
            X = layer(X)
        return X
...
class RIM_UNET_CELL(tf.nn.rnn_cell.RNNCell):
    ...
    def forward_pass(self, data):
        ...
        return output_series_1 , output_series_2 , final_log_L , dLdppar_series

    def cost_function( self, data,labels_x_1,labels_x_2 , starting_guess_1=None , starting_guess_2=None):
        if starting_guess_1 is not None: 
            self.inputs_1 = starting_guess_1
        if starting_guess_2 is not None:
            self.inputs_2 = starting_guess_2   
        output_series_1 , output_series_2 , final_log_L , dLdppar_series = self.forward_pass(data)
        return tf.reduce_mean(tf.square(output_series_1 - labels_x_1)) + tf.reduce_mean(tf.square(output_series_2 - labels_x_2)), output_series_1 , output_series_2 , output_series_1[-1].numpy() , output_series_2[-1].numpy() , dLdppar_series
...
class RIM_CELL(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs_1, state_1, grad_1, inputs_2, state_2, grad_2 , scope=None):
        xt_1, ht_1 = self.model_1(inputs_1, state_1 , grad_1)
        xt_2, ht_2 = self.model_2(inputs_2, state_2 , tf.zeros_like(grad_1))
        return xt_1, ht_1, xt_2, ht_2
    ...
    def cost_function( self, data,labels_x_1,labels_x_2 , starting_guess_1=None , starting_guess_2=None):
        ...
        return final_log_L + tf.reduce_mean(tf.square(output_series_1 - labels_x_1)) + tf.reduce_mean(tf.square(tf.math.exp(output_series_2) - tf.math.exp(labels_x_2))), output_series_1 , output_series_2 , output_series_1[-1].numpy() , output_series_2[-1].numpy() 
...
class Kappa_Model(tf.keras.Model):
    def __init__(self,num_cell_features):
        super(Kappa_Model, self).__init__()
        self.num_gru_features = num_cell_features/2
        num_filt_emb1_1 = self.num_gru_features
        num_filt_emb1_2 = self.num_gru_features
        num_filt_emb2 = self.num_gru_features
        ...
        self.gru2 = Conv_GRU(self.num_gru_features)
    def call(self, inputs, state, grad):
        inputs = tf.math.asinh(inputs)
        grad = tf.math.asinh(grad)        
        stacked_input = tf.concat([inputs , grad], axis=3)
        xt_1E = self.conv1_1(stacked_input)
        xt_1E = self.conv1_2(xt_1E)
        ht_11 , ht_12 = tf.split(state, 2, axis=3)
        ...
        delta_xt = self.conv4(delta_xt_1)
        #xt = delta_xt + inputs
        xt = tf.math.sinh(inputs) + tf.math.sinh(delta_xt)        
        ht = tf.concat([gru_1_out , gru_2_out], axis=3)
        return xt, ht



class KAPPA_RIM_UNET(tf.keras.Model):
    def __init__(self,num_cell_features):
        super(KAPPA_RIM_UNET, self).__init__()
        numfeat_1 , numfeat_2 , numfeat_3 , numfeat_4 = num_cell_features
        
        activation = lrelu4p
        self.STRIDE = 4
        self.Lc11 = tf.keras.layers.Conv2D(numfeat_1/2, (10, 10), activation=activation, padding='same') 
        self.Lp13 = tf.keras.layers.Conv2D(numfeat_1/2, (10, 10), activation=activation, strides=self.STRIDE, padding='same') 

        self.Lc21 = tf.keras.layers.Conv2D(numfeat_2/2, (10, 10), activation=activation, padding='same')
        self.Lc22 = tf.keras.layers.Conv2D(numfeat_2/2, (10, 10), activation=activation, padding='same')
        self.Lp23 = tf.keras.layers.Conv2D(numfeat_2/2, (10, 10), activation=activation, strides=self.STRIDE, padding='same')

        self.Lc31 = tf.keras.layers.Conv2D(numfeat_3/2, (10, 10), activation=activation, padding='same')
        self.Lc32 = tf.keras.layers.Conv2D(numfeat_3/2, (10, 10), activation=activation, padding='same')
        self.Lp33 = tf.keras.layers.Conv2D(numfeat_3/2, (7, 7), activation=activation, strides=self.STRIDE, padding='same')
        ...
        self.Lu71 = tf.keras.layers.Conv2DTranspose(numfeat_2/2, (6, 6), activation=activation, strides=self.STRIDE, padding='same')
        self.Lc72 = tf.keras.layers.Conv2D(numfeat_2/2, (10, 10), activation=activation, padding='same')
        self.Lc73 = tf.keras.layers.Conv2D(numfeat_2/2, (10, 10), activation=activation, padding='same')

        self.Lu81 = tf.keras.layers.Conv2DTranspose(numfeat_1/2, (6, 6), activation=activation, strides=self.STRIDE, padding='same')
        self.Lc82 = tf.keras.layers.Conv2D(numfeat_1/2, (10, 10), activation=activation, padding='same')
        self.Lc83 = tf.keras.layers.Conv2D(numfeat_1/2, (10, 10), activation=activation, padding='same')

        self.Loutputs = tf.keras.layers.Conv2D(1, (10, 10), activation='linear', padding='same')
...
class KAPPA_RIM_UNET_CELL(tf.nn.rnn_cell.RNNCell):
    def __init__(self, batch_size, num_steps ,num_pixels, state_size , input_size=None, activation=tf.tanh,cond1= None, cond2= None):
        self.num_pixels = num_pixels
        self.num_steps = num_steps
        self._num_units = state_size
        ...
        self.state_size_list_2 = [128 , 128 , 128 , 512]
        self.model_2 = KAPPA_RIM_UNET(self.state_size_list)
        self.batch_size = batch_size
        self.initial_condition(cond1, cond2)
        self.initial_output_state()
