...
def main(args):
    ...
    train_shards = train_items // args.example_per_shards + 1 * (train_items % args.example_per_shards > 0)
    for shard in range(train_shards):
        data = train_dataset.skip(shard * args.example_per_shards).take((shard + 1) * args.example_per_shards)
        with tf.io.TFRecordWriter(os.path.join(train_dir, f"data_{shard:20d}.tfrecords"), options=options) as writer:
            for example in data.map(decode_all):
                ...
                writer.write(record)
    val_shards = (total_items - train_items) // args.example_per_shards + 1 * ((total_items - train_items) % args.example_per_shards > 0)
    for shard in range(val_shards):
        data = val_dataset.skip(shard * args.example_per_shards).take((shard + 1) * args.example_per_shards)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--example_per_shard",      default=50000,  type=int,       help="Number of example to store in a single shard")
