...
def main(args):
    ...
    if args.cosmos_second_stage_vae is not None:
        with open(os.path.join(args.cosmos_second_stage_vae, "model_hparams.json"), "r") as f:
            cosmos_vae2_hparams = json.load(f)
        cosmos_vae2 = VAESecondStage(**cosmos_vae2_hparams)
        ckpt1 = tf.train.Checkpoint(step=tf.Variable(1), net=cosmos_vae2)
    ...
    with tf.io.TFRecordWriter(os.path.join(args.output_dir, f"data_{THIS_WORKER}.tfrecords"), options) as writer:
        ...
        for _ in range((THIS_WORKER - 1) * args.batch_size, args.len_dataset, N_WORKERS * args.batch_size):
            galaxies = cosmos_sampling_function(args.batch_size)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--cosmos_second_stage_vae",     default=None)
