    ...
    with h5py.File(os.path.join(os.getenv("CENSAI_PATH"), "results", args.output_name + ".h5"), 'w') as hf:
        ...
        for i in tqdm(range(args.sample_size)):
            ...
            for rank, j in enumerate(k_indices):
                ...
                g["matched_source"][rank] = source_d[..., 0].numpy().astype(np.float32)
                g["matched_kappa"][rank] = kappa_d[..., 0].numpy().astype(np.float32)
                g["matched_obs"][rank] = y_d[..., 0].numpy().astype(np.float32)
                g["matched_noise_rms"][rank] = noise_rms_d.numpy().astype(np.float32)
                g["matched_psf"][rank] = psf_d[..., 0].numpy().astype(np.float32)
