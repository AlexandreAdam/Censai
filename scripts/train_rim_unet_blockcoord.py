...
import numpy as np
import math
from censai import PhysicalModel, RIMUnetBlocCoord, RayTracer
...
def main(args):
    ...
    def train_step(X, source, kappa):
        ...
        if args.clipping:
            gradient1 = [tf.clip_by_norm(grad, 10.) for grad in gradient1]
            gradient2 = [tf.clip_by_norm(grad, 10.) for grad in gradient2]
