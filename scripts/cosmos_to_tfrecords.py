...
if args.do_all:
    print(f"Started do_all job at {datetime.now().strftime('%y-%m-%d_%H-%M-%S')}")
    if args.sample == "23.5":
        n_tasks = 58
    ...
    for task_id in range(0, n_tasks):
        print(f"Started task {task_id} at {datetime.now().strftime('%y-%m-%d_%H-%M-%S')}")
        ...
        with tf.io.TFRecordWriter(filename, options=options) as writer:
            for record in gen:
                writer.write(record)
        print(f"Finished task {task_id} at {datetime.now().strftime('%y-%m-%d_%H-%M-%S')}")
