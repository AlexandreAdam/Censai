...
def distributed_strategy(args):
    ...
    ps_observation = PowerSpectrum(bins=args.observation_coherence_bins, pixels=pixels)
    ...
    with h5py.File(os.path.join(os.getenv("CENSAI_PATH"), "results", args.experiment_name + "_" + model_name + f"_{THIS_WORKER:02d}.h5"), 'w') as hf:
        ...
        hf.create_dataset(name="psf",                                    shape=[data_len, psf_pixels, psf_pixels, 1],  dtype=np.float32)
        ...
        hf.create_dataset(name="observation_prior",                             shape=[data_len, phys.pixels, phys.pixels, 1],                                      dtype=np.float32)
        ...
        hf.create_dataset(name="source_prior",                           shape=[data_len, phys.src_pixels, phys.src_pixels, 1],                              dtype=np.float32)
        ...
        hf.create_dataset(name="kappa_prior",                            shape=[data_len, phys.kappa_pixels, phys.kappa_pixels, 1],                          dtype=np.float32)
        ...
        for i in range(data_len):
            ...
            hf["observation_prior"][i] = sampled_observation[0].numpy().astype(np.float32)
            ...
            hf["source_prior"][i] = sampled_source[0].numpy().astype(np.float32)
            ...
            hf["kappa_prior"][i] = 10**sampled_kappa[0].numpy().astype(np.float32)
