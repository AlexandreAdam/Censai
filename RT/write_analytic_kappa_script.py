...
lens_util_obj = lens_util(numpix_side = num_features , im_side= 20.48, method="conv2d",  kap_side=20.48)

print kappa.shape
data_size = len(kappa[:,0,0])

with tf.device('/gpu:0'):
    for ind in range(data_size):
        print ind
        batch_kappa = kappa[ind,:].reshape((-1,num_features,num_features,1))
        ...
        np.save('defangles/x_a_labels' + str(ind) + '.npy', x_a_labels.numpy() )
        np.save('defangles/y_a_labels' + str(ind) + '.npy', y_a_labels.numpy() )
