...
dataset = files.interleave(lambda x: tf.data.TFRecordDataset(x), block_length=1, num_parallel_calls=tf.data.AUTOTUNE)
...
# Test that each element appear once when we go through the dataset
# dataset = dataset.map(decode)
# recovered_data = []
# for x in dataset.as_numpy_iterator():
#     recovered_data.append(x)
# a = set(recovered_data)
# true = set(range(20000))
# print(true.difference(a)) # should be empty set if we did everything right

# Test that the dataset is shuffled differently each calls
# recovered_data2 = []
# for x in dataset.as_numpy_iterator():
...
# Test that subsample of the dataset can recover large parts of the data
recovered_data3 = []
...
print(len(np.unique(recovered_data3))) # should be close
