...
class RIM_UNET(tf.keras.Model):
    def __init__(self,num_cell_features):
        ...
        self.CDC = RIM_COMP_DECOMP()
        self.GRU_COMP = GRU_COMPONENT(numfeat_3)
    ...
    def call(self, inputs, state, grad):
        ...
        embedded = self.CDC.COMP(stacked_input)
        c3_gru , c3_gru_state = self.GRU_COMP(embedded , ht_3)
        decodded = self.CDC.DECOMP(c3_gru)
        
        

        delta_xt = self.Loutputs(decodded)
        xt = inputs + delta_xt
        ht = [ht_1 , ht_2 , c3_gru_state , ht_4]
...
class RIM_UNET_CELL(tf.nn.rnn_cell.RNNCell):
    def __init__(self, batch_size, num_steps ,num_pixels, state_size , input_size=None, activation=tf.tanh):
        ...
        self.state_size_list = [64 , 64 , 128 , 64]
...
class SRC_KAPPA_Generator(object):
    ...
    def draw_k_s(self,train_or_test):
        ...
        for i in range(num_samples):
            ...
            sigma_src = np.random.uniform(low=0.2, high=1.4)
            x_src = np.random.uniform(low=-4.5, high=4.5)
            y_src = np.random.uniform(low=-4.5, high=4.5)
            norm_source = True
...
class lens_util(object):
    ...
    def get_deflection_angles(self, Kappa):
        ...
        if (self.method=="conv2d"):
            ...
            alpha_x = tf.zeros_like(Kappa)
            alpha_y = tf.zeros_like(Kappa)
    ...
    def AE_physical_model(self, Src , Kappa):
        Src = self.AE.decode(Src)
        Kappa = self.AE.decode(Kappa)
        return self.physical_model(Src , Kappa)
