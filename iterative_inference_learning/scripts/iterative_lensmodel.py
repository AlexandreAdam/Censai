...
import sys
sys.path.append('/Users/wmorning/class/')
from CS230 import SrcLikelihood
...
global_step = tf.Variable(0, trainable=False, dtype=tf.int32)
x = tf.placeholder(dtype=tf.float32,shape=[None,numpix_side**2])
x_image = tf.reshape(x,[-1,numpix_side,numpix_side,1])
x_init = tf.zeros_like(x_image)
...
likelihoodobj = SrcLikelihood(lens_model,x_image,numpix_side,0.04,numpix_src,0.014150943396226415)
def mse(x_test):
    return likelihoodobj.Loglikelihood(x_test)
...
def lossfun(x_est,names):
    return tf.reduce_sum(-0.5 * tf.square(names(x_est,'mu',4),y_image))
