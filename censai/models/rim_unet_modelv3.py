...
from censai.models.layers import UnetDecodingLayer, UnetEncodingLayer, ConvGRUPlus, ConvGRU, ConvGRUPlusHighway
    ...
    def init_hidden_states(self, input_pixels, batch_size, constant=0.):
        ...
        for i in range(self._num_layers):
            ...
            hidden_states.append(
                constant * tf.ones(shape=[batch_size, pixels, pixels, filters], dtype=DTYPE)
            )
        pixels = input_pixels // self._strides ** (self._num_layers)
        hidden_states.append(
            constant * tf.ones(shape=[batch_size, pixels, pixels, self._bottleneck_filters], dtype=DTYPE)
        )
