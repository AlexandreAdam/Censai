...
# Only difference with v3 is that y_true - y_pred is passed to unet instead of y_true only
class RIMSharedUnetv4:
    def __init__(
            self,
    ...
    def time_step(self, residuals, source, kappa, source_grad, kappa_grad, states, training=True):
        x = tf.concat([residuals, source, kappa, source_grad, kappa_grad], axis=3)
    ...
    def call(self, lensed_image, noise_rms, psf, outer_tape=nulltape):
        ...
        source, kappa, states = self.time_step(lensed_image / noise_rms[:, None, None, None], source, kappa, source_grad, kappa_grad, states)  # Use lens to make an initial guess with Unet
        ...
        for current_step in tf.range(self.steps-1):
            ...
            source, kappa, states = self.time_step((lensed_image - y_pred) / noise_rms[:, None, None, None], source, kappa, source_grad, kappa_grad, states)
    ...
    def predict(self, lensed_image, noise_rms, psf):
        ...
        for current_step in range(self.steps-1):
            ...
            source_grad, kappa_grad = g.gradient(cost, [source, kappa])
            source_grad, kappa_grad = self.grad_update(source_grad, kappa_grad, current_step)
            source, kappa, states = self.time_step((lensed_image - y_pred) / noise_rms[:, None, None, None], source, kappa, source_grad, kappa_grad, states, training=False)
