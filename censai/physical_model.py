...
class AnalyticalPhysicalModel: 
    ...
    def kappa_field(self, r_ein=1., e=0., phi=0., x0=0., y0=0.):
        theta1, theta2 = self.rotated_and_shifted_coords(phi, x0, y0)
        return 0.5 * r_ein / tf.sqrt(theta1**2/(1-e) + (1-e)*theta2**2 + self.theta_c**2)

    def lens_source(self, source, r_ein, e=0., phi=0., x0=0., y0=0., gamma_ext=0., phi_ext=0.):
        if e < 0.1:
            alpha1, alpha2 = tf.split(self.approximate_deflection_angles(x0, y0, e, phi, r_ein), 2, axis=-1)
        else:
            alpha1, alpha2 = tf.split(self.analytical_deflection_angles(x0, y0, e, phi, r_ein), 2, axis=-1)
        alpha1_ext, alpha2_ext = self.external_shear_deflection(gamma_ext, phi_ext)
        theta1, theta2 = self.theta1 - x0, self.theta2 - x0
        # lens equation
        beta1 = theta1 - alpha1 - alpha1_ext
        beta2 = theta2 - alpha2 - alpha2_ext
    ...
    def lens_source_func(self, r_ein=1., e=0., phi=0., x0=0., y0=0., gamma_ext=0., phi_ext=0., xs=0., ys=0., es=0., w=0.1):
        if e < 0.1:
            alpha1, alpha2 = tf.split(self.approximate_deflection_angles(x0, y0, e, phi, r_ein), 2, axis=-1)
        else:
            alpha1, alpha2 = tf.split(self.analytical_deflection_angles(x0, y0, e, phi, r_ein), 2, axis=-1)
    ...
    def noisy_lens_source(self, source, noise_rms=1e-3, r_ein=1., e=0., phi=0., x0=0., y0=0., gamma_ext=0., phi_ext=0.):
    ...
    def approximate_deflection_angles(self, xlens, ylens, elp, phi, r_ein):
        ...
        denominator = (theta1 ** 2 / (1 - elp) + theta2 ** 2 * (1 - elp) + self.theta_c ** 2) ** (1 / 2)
