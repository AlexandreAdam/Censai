...
class PowerSpectrum:
    ...
    def cross_correlation_coefficient(self, x, y):
        ...
        ps = tf.TensorArray(DTYPE, size=self.bins)
        for i in range(self.bins):
            Sxy = tf.reduce_sum(tf.abs(tf.math.conj(x_hat) * y_hat) * self.masks[i][None, ...], axis=(1, 2, 3)) / (self.pixels//2)**2 / (4 * np.pi)
            Sxx = tf.reduce_sum(tf.abs(x_hat)**2 * self.masks[i][None, ...], axis=(1, 2, 3)) / (self.pixels//2)**2 / (4 * np.pi)
            Syy = tf.reduce_sum(tf.abs(y_hat)**2 * self.masks[i][None, ...], axis=(1, 2, 3)) / (self.pixels//2)**2 / (4 * np.pi)
            value = Sxy / tf.sqrt(Sxx * Syy) / tf.reduce_sum(self.masks[i])
            ps = ps.write(index=i, value=value)
        ps = tf.transpose(ps.stack())  # reshape into [batch_size, bins]
        return ps
...
if __name__ == '__main__':
    ...
    circle2 = np.cos(1 * np.pi * r / 3)
    y = x + 10 * tf.cast(circle2[np.newaxis, ..., np.newaxis], tf.float32)
    ell = ps.power_spectrum(x)
    # ps.plot_power_spectrum_statistics(x, 10)
    # ps.plot_power_spectrum(x, 10)
    # plt.show()

    ps.plot_cross_power_spectrum_statistics(x, x, 10)
    plt.show()
